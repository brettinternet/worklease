...
4. Derive the source-scoped resource and acquire it through `LeaseStore`.
...
Verify the resulting source bytes and provider checkpoint before calling `store.release` with `updated_claim` and a new release operation ID. The release reason records what was verified; it does not prove verification.
