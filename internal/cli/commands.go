...
func newCommands(s *boundary) []*urfavecli.Command {
	...
	historyCommand := jsonless("history", "show recent lifecycle events or retained resource history", "worklease history\n  worklease history --resource RESOURCE", resources(), flag("cursor"), &urfavecli.IntFlag{Name: "limit", Usage: "limit"}, full("show complete identifiers, operation metadata, and absolute timestamps"))
	historyCommand.Action = historyAction(s)
	textOutput(historyCommand, "Without a resource the command shows the global lifecycle event feed; --resource shows retained claim epochs for exactly one resource. --full adds complete non-secret metadata and RFC3339 timestamps.")
