...
func writeLedgerResult(s *boundary, cmd *urfave.Command, operation string, fields map[string]any) error {
	if s.jsonRequested(cmd) {
		...
		if operation == "inspect" && cmd.Bool("full") {
			return output.WriteSuccess(s.writer, operation, public)
		}
		return output.WritePublicSuccess(s.writer, operation, public)
	}
	if operation == "history" {
		return writeHistoryText(s.writer, fields["historyPage"].(ledger.HistoryPage), cmd.Bool("full"), output.ColorEnabled(s.writer))
	}
	if operation == "events" {
		return writeEventsText(s.writer, fields["eventsPage"].(ledger.EventsPage), cmd.Bool("full"), output.ColorEnabled(s.writer))
	}
	if operation == "inspect" {
		return writeInspectionText(s.writer, fields["inspection"].(ledger.Operation))
	}
	if operation == "reconcile" {
		return writeReconciliationText(s.writer, fields["receipt"].(lease.ReconciliationReceipt))
	}
