...
var topics = map[string][]string{
	...
	"loop": {
		...
		"2. Acquire immediately after choosing a candidate, before reading its full intent, planning, delegation, or edits. For a configured queue, use worklease queue next --view NAME --claim --session ID --json to select and acquire in one step. It skips contended candidates without waiting and returns one worker-owned contextual claim; on uncertain acquisition stop and recover the pending handle. For other sources, on conflict do not wait: select the next ready candidate. Hold at most one claim while selecting.",
		"3. Give each independent loop one stable unique identity. Use a harness-provided loop-run ID unchanged; if none exists, generate a full UUID and persist it before work. For CLI, set it as WORKLEASE_SESSION_ID. For MCP, pass it as acquire.sessionId, persist the returned lease before work, and use that lease for later lifecycle calls. Never truncate or derive the identity from agent names, labels, or other non-unique metadata.",
