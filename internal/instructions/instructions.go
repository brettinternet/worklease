...
var topics = map[string][]string{
	...
	"loop": {
		...
		"2. Acquire immediately after choosing a candidate, before reading its full intent, planning, delegation, or edits. For a configured queue, use worklease queue next --view NAME --claim --session ID --json or MCP queue_next with view, claim: true, and sessionId: ID to select and acquire in one step. Both skip contended candidates without waiting. The CLI returns a contextual claim; MCP returns an opaque lease for verify, heartbeat, checkpoint, and release. On uncertain acquisition stop and recover the pending handle or MCP lease reference. For other sources, on conflict do not wait: select the next ready candidate. Hold at most one claim while selecting.",
		"3. Give each independent loop one stable unique identity. Use a harness-provided loop-run ID unchanged; if none exists, generate a full UUID and persist it before work. For CLI, set it as WORKLEASE_SESSION_ID. For MCP, pass it as acquire.sessionId, persist the returned lease before work, and use that lease for later lifecycle calls. queue_next.sessionId uses the same stable identity and lease lifecycle. Never truncate or derive the identity from agent names, labels, or other non-unique metadata.",
		"4. Use --handle PATH only when an explicit private CLI handle is required. Heartbeat before half the TTL and around long work. MCP automatic heartbeat is process-scoped and may stop on disconnect, server shutdown, or loop replacement; never rely on it across agent turns, waits, or loop iterations.",
		"5. Before blocking work, verify ownership and ensure the operation is bounded to finish before expiry with margin; otherwise heartbeat first and shorten or split it. Long mutating work requires a guarded operation or another mechanism that maintains ownership.",
		"6. After every subagent run, wait, long command, human pause, resumed session, or new loop iteration, verify the exact claim and refresh authoritative provider state before any filesystem edit, mutating command, provider write, commit, merge, or cleanup.",
		"7. On stale-claim, claim-expired, or ownership-lost, stop without further mutation. Claim expiry does not prove the prior worker stopped and does not authorize resuming in-progress work; require explicit handoff or authoritative abandonment evidence.",
