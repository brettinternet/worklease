...
class LeaseStore:
    ...
    def _validate_reconciliation_replay(
        receipt: dict[str, Any], request: MutationRequest | BundleMutationRequest
    ) -> None:
    ...
    def reconcile_bundle_operation(
        ...
        """Record an observed result for one started ordered-bundle operation."""
        ...
        with (
            resource_locks(request.resources, self.home),
            closing(self._connect()) as db,
            transaction(db),
        ):
            owner = self._require_bundle_owner(db, request)
            if self.clock() >= float(owner["expires_at"]):
                raise LeaseError(
                    "claim-expired",
                    resource=",".join(request.resources),
                    claim=self._bundle_claim(db, owner).to_dict(include_token=False),
                )
            replay = db.execute(
                """
                SELECT * FROM reconciliations
            ...
            if not target_rows:
                raise LeaseError(
                    "operation-not-found", code=3, operationId=target_operation_id
                )
            if len(target_rows) != 1:
                raise LeaseError(
                    "operation-id-ambiguous",
            ...
            receipt: dict[str, Any] = {
                "ok": True,
                "operation": "reconcile-operation-bundle",
                "operationId": request.operation_id,
                "targetOperationId": target_operation_id,
                "resources": list(request.resources),
                "state": "reconciled",
            ...
            current = self._bundle_row(db, request.claim_id)
            if current is None:
                raise LeaseError(
