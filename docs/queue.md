...
The read-only built-in `linear` source uses `organization: ORG_UUID`, `team: TEAM_UUID`, optional `project: PROJECT_UUID`, `account: VIEWER_UUID`, and `credentialHelper: [/absolute/path/to/helper, arg]`. `organization` is the stable source identity, `team`/`project` restrict discovery, and issue UUIDs (not identifiers such as `TEST-1`) identify items. The adapter verifies the authenticated viewer, organization and team before reading. It pages issues and both directions of relations at no more than 250 nodes per request; moving `updatedAt` pages, archived/trashed issues, and unprobed partial visibility leave source coverage partial until the separate synchronization/reconciliation work lands. Dependency edges from a different team or excluded project remain unknown rather than being silently treated as satisfied. Claims and writes are disabled; `claims`/`workflow` are rejected until their separate implementation. For forthcoming `jira-cloud` sources, owner-private `queue.yaml` accepts `account: EXPECTED_PRINCIPAL` and the same credential helper configuration. The helper prints **one token line** to stdout; put credentials in the helper's private store, not in YAML or argv. Execution has a 15-second timeout and a 4 KiB limit on each output stream, terminates its process group on exit, inherits only process plumbing and the user's private configuration directory variables, and discards stderr. The queue resolves each credential serially and returns a token only after the adapter verifies its authenticated principal against `account`; failures and mismatches disable writes without echoing secrets. GitHub continues using `gh auth token` unchanged. Jira Cloud is not yet shipped: configuring it now does not enable reads, claims, or writes. Linear read-only access requires explicit configuration and network access; no provider mutations are sent.
