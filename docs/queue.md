...
sources:
  - id: planning
    ...
    workflow: {start: in_progress, blocked: blocked, complete: closed, reopen: open}
views:
  - name: Ready
    authority: local
    sources: [planning]
...
The `generic` source name is a portable binding: every worker must use the exact same `worklease key --provider generic --source agreed-team/planning --item BEAD-ID` bytes and authority. Stop old workers, resolve old claims/operations, and run `worklease queue --view Ready identity confirm --source planning --acknowledge` before claim actions; a claim does **not** synchronize Beads databases across clones. Duplicate or disappeared confirmed IDs disable claiming until the source is reconciled and rebound. `bd` assignment and `in_progress` status are provider display state, not Worklease claims. The queue uses complete bulk JSON lists for typed blocking dependencies; `closed` and the configured `workflow.complete` state satisfy terminal prerequisites. It shows `bd` ready separately from queue readiness and polls for changes every 15 seconds. An unreadable or partial list cannot prove deletion.

Local-only reads require embedded Dolt and no configured `sync.remote`, and run `bd --readonly --sandbox` without remote access. If a Dolt remote is configured, set `allowGitNetwork: true` only after consenting to possible provider network effects; the adapter still disables auto-push and never runs `bd sync`. Server mode is unsupported. A focused Beads write previews and creates a **local Dolt commit**, never a Git commit or Git hook invocation. If `export.auto=true`, a throttled `.beads/issues.jsonl` export may change a Git-tracked file; the preview discloses it. Unrelated staged Git changes remain staged. Read-back verifies the exact issue/comment marker and resulting Dolt commit before checkpointing; if a later Dolt commit obscures attribution, verification stays unknown and requires operator recovery. A lost response is never re-dispatched.

The built-in `linear` source uses `organization: ORG_UUID`, `team: TEAM_UUID`, optional `project: PROJECT_UUID`, `account: VIEWER_UUID`, and `credentialHelper: [/absolute/path/to/helper, arg]`. `organization` is the stable source identity, `team`/`project` restrict discovery, and issue UUIDs (not identifiers such as `TEST-1`) identify items. The adapter verifies the authenticated viewer, organization and team before reading and before every write. It pages issues and both directions of relations at no more than 250 nodes per request; moving `updatedAt` pages, archived/trashed issues, and unprobed partial visibility leave source coverage partial until synchronization/reconciliation completes. Dependency edges from a different team or excluded project remain unknown rather than being silently treated as satisfied. Linear Worklease claims are supported; provider-native claims are not.

Linear writes are optional configured transitions and focused operations. Map workflow actions to explicit UUIDs for states belonging to the bound team; the adapter rediscovers the selected UUID and validates its state type before writing. `start` must map to type `started` (never inferred from a state display name); `blocked`/`review` require `started`, `complete` requires `completed`, and `reopen` requires `backlog` or `unstarted`. For example:

```yaml
workflow:
  start: 11111111-1111-4111-8111-111111111111 # team state UUID, type started
  blocked: 22222222-2222-4222-8222-222222222222
  review: 33333333-3333-4333-8333-333333333333
  complete: 44444444-4444-4444-8444-444444444444
  reopen: 55555555-5555-4555-8555-555555555555
```

The TUI's configured-state action, explicit Start work, Record progress, and Assign to me use the journaled write pipeline. Progress uses `commentCreate` with an operation marker and verifies the exact comment, actor, and marker through `issue.comments`; duplicate or copied markers remain ambiguous. Linear has one assignee slot: the preview names the current assignee and explicitly asks confirmation before replacing someone else. Assign-to-me is not a claim. Helper argv, organization/team/project binding, account, and relevant workflow mapping changes invalidate a pending preview or recovery; restore the original binding to continue read-only recovery. Requests are serialized per organization/account, respect both request and complexity quotas, and never redispatch an unresolved mutation. Lost responses and lagged read-back remain in recovery until provider evidence is sufficient; a marker is correlation, not idempotency.

For forthcoming `jira-cloud` sources, owner-private `queue.yaml` accepts `account: EXPECTED_PRINCIPAL` and the same credential helper configuration. The helper prints **one token line** to stdout; put credentials in the helper's private store, not in YAML or argv. Execution has a 15-second timeout and a 4 KiB limit on each output stream, terminates its process group on exit, inherits only process plumbing and the user's private configuration directory variables, and discards stderr. The queue resolves each credential serially and returns a token only after the adapter verifies its authenticated principal against `account`; failures and mismatches disable writes without echoing secrets. GitHub continues using `gh auth token` unchanged. Jira Cloud is not yet shipped: configuring it now does not enable reads, claims, or writes.

External adapters can also use the helper with an approved `account` and canonical HTTPS `config.origin`; they declare `host-credential-v1` and receive replacements on the credential protocol method before each operation. See the [external adapter protocol](external-adapter-protocol.md) for approval and scope requirements.

`workflow` is an optional per-source map from `start`, `blocked`, `review`, `complete`, and `reopen` to real provider transitions. Configure only transitions that the provider supports; the write adapter validates each mapping before a write. An absent mapping reports `no-workflow-mapping` rather than inventing a provider status. The configured account is the external adapter's principal for capability checks and assignment; it does not alter the source-scoped generic claim identity. The TUI's `s` (configured state), `p` (progress note), and `a` (assign to me) actions require an existing verified queue-owned claim. Each shows the exact provider effect, claim, races, side effects, and marker before confirmation; the claim alone never changes provider state. No status mapping is invented for an unsupported provider. Pending write intents are stored under `$XDG_STATE_HOME/worklease/queue-recovery/` (default `~/.local/state/worklease/queue-recovery/`), apart from the disposable index. Do not delete unresolved records when clearing the cache or logging out. `worklease queue recovery --json` lists unresolved records across sources with the operation ID, item, intended effect, claim, dispatch time, last read-back outcome, and permitted recovery steps; it reads only owner-private state and does not need a queue view or live provider. In the TUI, the Recovery view lists the same records and the item detail's Recovery tab filters them by item. A lost write response is never success or a reason to release the claim: `worklease queue recovery retry --operation-id ID --handle PRIVATE_HANDLE` retries read-back and any exact checkpoint replay without redispatching the provider write. If read-back fails, its error reports `commitState: unknown` until the outcome is proven; an absent recovery ID reports `operation-not-found`, not proof that a provider write did not commit. Select the original authority with `--profile NAME` if it is not the default. For a verified provider effect with a pending checkpoint, `retry` checks the original authority first: a committed checkpoint resolves normally, but after `CheckpointNotAfter` it will not replay an absent checkpoint. After confirming no executor can still commit it, run `worklease queue recovery checkpoint-missing --operation-id ID --handle PRIVATE_HANDLE --evidence 'what was checked' --provider-verified --checkpoint-absent --executor-ceased` against the original authority. It rechecks checkpoint status after the original authority's conservative time bound has passed the deadline (the client clock alone is insufficient), requires durable verified provider read-back, and records the operator identity and typed evidence in the terminal `checkpoint-missing` journal record. In the TUI Recovery view, press `e` on a checkpoint-pending record and type `PROVIDER VERIFIED; CHECKPOINT ABSENT; EXECUTOR STOPPED: ` followed by provider and authority audit evidence. Neither path redispatches the provider write or replays an expired checkpoint. The provider effect happened, the Worklease checkpoint did not; the claim remains held and cannot be cancelled as no-effect. This terminal outcome no longer blocks later writes on the same item. If authority status is unavailable or the executor may still finish, leave the record unresolved. Once the original executor has stopped and a provider audit proves no write committed, `worklease queue recovery reconcile --operation-id ID --evidence 'what was checked' --no-commit --executor-ceased` records the OS operator identity and typed evidence in the private journal. Reconciliation is refused while dispatch is still in progress or a provider receipt/verified read-back exists; it does not checkpoint or release the claim.
