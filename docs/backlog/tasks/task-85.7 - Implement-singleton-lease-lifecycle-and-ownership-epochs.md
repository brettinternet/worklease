...
- [ ] #2 Credential tests prove the check order of contract 7.3 (unknown claim id gives stale-claim, wrong token gives invalid-token, expired claim gives claim-expired, wrong revision gives stale-revision with expectedRevision and suppliedRevision), that none of these failures mutate rows or append events, that omitting --revision on a mutating command fails invalid-argument while status accepts explicit credentials without it, and that an out-of-bounds ttl or a checkpoint over 8 KiB fails before any write.
- [ ] #3 Idempotency tests prove replaying an operation id with the same request returns the stored receipt with idempotent true and no revision change, a changed request (including a different ttl) fails operation-request-mismatch, a started operation created through BeginOperation fails unknown-outcome on replay, an acquire replay with the same claim id and an identical request returns the claim without the token and with idempotent true, and an acquire replay with the same claim id and a different ttl fails operation-request-mismatch.
