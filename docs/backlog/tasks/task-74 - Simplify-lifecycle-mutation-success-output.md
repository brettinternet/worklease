...
- [ ] #3 Acquire and transfer continue to emit the bearer token when no lease handle file is used; handle-file flows clearly identify the usable handle without exposing its contents. No other success or failure path exposes bearer tokens.
...
- [ ] #7 CLI contract tests cover every canonical singleton and bundle mutation, aliases where relevant, token and lease-file flows, child success/failure/truncation, empty optional fields, full-detail behavior, JSON compatibility, and documentation.
- [ ] #8 A consistent documented `--full` text option on canonical singleton and bundle mutation commands, and their aliases, preserves all current non-secret text fields; JSON schemas and payloads remain compatible.
<!-- AC:END -->
