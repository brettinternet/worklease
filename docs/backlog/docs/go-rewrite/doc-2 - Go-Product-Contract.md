...
This document is the normative design for TASK-85 (Rewrite Worklease in Go), every TASK-85.x subtask, and the planned TASK-107 remote authority slice governed by section 20. It exists so that implementing agents working unattended do not have to make product, architecture, or interface decisions. Read it completely before starting any covered task, then re-read the sections named in your task.
...
- The Go implementation is POSIX-only (Linux and macOS). Shipped V1 coordinates cooperating processes using the same authority on the same host; arbitrary child processes and direct provider writes are not fenced. Section 20 authorizes the planned TASK-107 remote slice without changing current local behavior or claiming that remote implementation is shipped.
...
## 20. Remote authority boundary (TASK-107 authorized; not shipped)

### 20.1 Scope and deployment

The shipped release remains the local POSIX SQLite authority. This section authorizes only the planned TASK-107 remote slice; it does not claim that an HTTP client, server, authentication flow, deployment, or remote guarantee exists. Historical local behavior, including D7/D8 and local `--no-handle`, remains unchanged except for the shared schema migration when the remote-capable store is implemented. The future standard binary opens no listener and performs no network work unless the user selects a remote profile, invokes explicit remote profile management, or runs `worklease serve`.

The remote slice serves the existing Go `lease.Service` over authenticated HTTPS from one always-on host, one persistent SQLite volume, and one namespace per process. One serialized SQLite writer remains the namespace throughput boundary. A deployment-owned configuration file lists delimiter-terminated portable prefixes and TTL/hold bounds; known host-local prefixes remain forbidden, and configuration changes take effect only after stop/start. A trusted reverse proxy may terminate TLS but sees bearer secrets; proxy identity headers grant no authority. Optional asynchronous object backup is disaster recovery, not high availability. Do not add a backend registry, remote process runner, provisioning system, or second claim implementation. `docs/remote-claim-authority.md` records the detailed chosen design, rejected alternatives, and follow-up triggers.
