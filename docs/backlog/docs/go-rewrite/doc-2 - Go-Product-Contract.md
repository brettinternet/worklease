...
The deferred design in `docs/remote-claim-authority.md` recommends serving this same Go authority (`lease.Service` plus the SQLite store) over authenticated HTTPS from one always-on host with one volume and optional asynchronous replication to object storage, one namespace per `serve` process, and portable keys only, with admitted prefixes and TTL/hold bounds in a deployment-owned server configuration file. A reverse proxy or tunnel may terminate TLS, but Worklease owns installation credentials, roles, and API authorization; no proxy identity header is an authorization source. Installations enroll by redeeming short-lived one-shot invites with client-generated credentials that the authority stores only as hashes, under `read`, `write`, and `admin` roles; there is no OAuth server, browser login, or session. A Durable Object reimplementation was rejected because it duplicates every safety invariant in a second language. Future authority authentication is separate from each claim credential; configured remote failures must never fall back to local claims. Multi-host transport must preserve exact request/replay and unknown-outcome semantics and explicitly handle network partitions. A hosted deployment adds two invariants before release. First, a random restore incarnation identifier (`restoreId`) in `meta`, regenerated on every restore and never on ordinary restart: every authenticated request carries it as an immutable `expectedRestoreId` checked in the serialized boundary before replay or mutation, every response returns the current value, cursors embed it, and receipts and handles retain it as provenance, so a stale request fails `authority-restored` and a restore fails closed into a namespace recovery mode that refuses new guarded effects and ordinary admission until an operator reopens it. Recovery mode is a namespace state, not a claim type. The client's recovery evidence is its pending state, cleared only on a confirmed terminal response. Second, an exclusive process-lifetime OS lock on the hosted volume, so one database is never served by two live processes; it does not change the local CLI's concurrency model. Adding `restoreId` and the `restored` and `revoked` epoch end reasons changes the shared SQLite schema and requires a `SchemaVersion` bump with a one-way migration when authorized. Neither invariant exists today and neither is added before the feature is authorized.
