...
This document is the normative design for TASK-85 (Rewrite Worklease in Go), every TASK-85.x subtask, and the experimental TASK-107 remote authority capability governed by section 20. It exists so that implementing agents working unattended do not have to make product, architecture, or interface decisions. Read it completely before starting any covered task, then re-read the sections named in your task.
...
- Worklease is a work-lease coordinator for humans and coding agents. Its default local authority coordinates one host; the opt-in experimental remote authority coordinates cooperating clients across hosts. It grants one worker a bounded ownership epoch over one or more opaque resources, guards client-local operations behind that ownership, and keeps a redacted authority history. The external work provider (Backlog.md, GitHub, Linear, Markdown files) remains authoritative for item state and progress.
...
- The Go implementation is POSIX-only (Linux and macOS). The default local authority coordinates cooperating processes on one host. The opt-in experimental remote authority in section 20 coordinates cooperating clients across hosts while guarded effects remain client-local; arbitrary child processes and direct provider writes are not fenced. Local behavior remains unchanged apart from the shared schema migration.
...
- 2026-09-13, owner-requested remote design refinement (no task): section 20 replaces the Cloudflare Tunnel plus Access front door with a TLS-only edge and Worklease-owned invite-based installation authentication with `read`/`write`/`admin` roles; fixes the initial remote scope to one namespace per process and portable keys with server-configured bounds; states the `restoreId` contract as an immutable `expectedRestoreId` in every authenticated request checked before replay or mutation, `restoreId` on every response and in cursors, and provenance in receipts and handles; replaces the quarantined recovery-only claim type with a namespace recovery mode; scopes the process-lifetime lock to the hosted volume; names client pending state as the recovery evidence; and records that the hosted invariants require a shared schema version bump. No current decision, command, or shipped behavior changes; remote implementation remains deferred. Evidence: the owner's decisions and the rejected alternatives are recorded in `docs/remote-claim-authority.md`, including the R0-to-R1 stale-request counterexample that requires request-side incarnation binding rather than response-only detection, and the pending-state argument that defers a mandatory client journal to a follow-up; `internal/store/schema.go` CHECK-constrains `epochs.end_reason` to `released`, `transferred`, and `expired`, which is why the schema bump is named.

- 2026-09-13, TASK-107, owner-requested remote design finalization: section 20 now authorizes the planned remote implementation while preserving shipped local behavior; it defines the Go/SQLite single-writer slice, explicit protocol freeze, Worklease-owned invite authentication, immutable request incarnation binding, durable exact pending state for every remote mutation, namespace recovery, offline administrative boundaries, hosted-home writer exclusion, shared schema version 2, and evidence required before reopening. The contract introduction and section 1 now name TASK-107 as the remote implementation authority without changing current V1. Evidence: `internal/cli/guard_commands.go` permits `guardLifecycle` to be nil for local `--no-handle`, so remote handleless recovery is a scoped future override; `internal/guard/guard.go` completes after the supervised child exits and does not establish provider or escaped-descendant cessation; selecting a backup older than the monitored tip shows why measured replication lag cannot bound the actual lost-history interval; and the existing TASK-107 family already owns protocol, schema, server, client, MCP, restore, and deployment evidence. Full rationale and alternatives remain in `docs/remote-claim-authority.md`.

- 2026-09-14, TASK-107.1 remote protocol freeze: section 20.3 now freezes [Remote Authority Protocol V1](../remote-authority/doc-4%20-%20Remote-Authority-Protocol-V1.md) by reference. The specification fixes `worklease-http/1`, exact HTTPS routes and CLI flags, storage-neutral wire mappings to the existing lease/ledger/watch/GC types and named authentication/administration extensions, request/response bounds, cancellation and non-cacheability, installation/invite/epoch authentication precedence, incarnation-bound cursors, exact replay before new-admission checks, the complete error-to-HTTP map, and the offline/client/HTTP/CLI/MCP exposure matrix. It excludes `LegacyRequestHash`, remote replacement and provider execution, HTTP retirement, MCP enrollment, recovery import, and cross-host transfer. Evidence: `internal/lease/service.go`, `internal/lease/reconciliation.go`, `internal/ledger/ledger.go`, `internal/watch/watch.go`, `internal/gc/gc.go`, `internal/reason/reason.go`, the TASK-107 design decisions in `docs/remote-claim-authority.md`, and TASK-107.1 acceptance review.

- 2026-09-15, TASK-107.12 experimental remote promotion: the introduction, sections 1 and 16, and section 20 now describe the implemented opt-in remote authority rather than a future slice. The standard binary remains local and network-inert unless remote profile management, a selected remote profile, or `serve` is explicitly invoked; guarded and provider effects stay client-local. One process serves one namespace through one SQLite writer and the hosted lock on a single-host filesystem. Restore reopening retains the full independent inventory, pending-set, retained/lost-tail outcome, provider/executor cessation, selected-cutoff, loss-interval, unknown-bound, and private-attestation requirements; missing coverage keeps recovery closed indefinitely. Remote replacement/provider execution, recovery import, completed-history journaling, cross-host transfer, repository enrollment, high availability, Postgres, multi-namespace serving, backpressure, and browser control-plane work remain deferred with the triggers in `docs/remote-claim-authority.md`. Evidence: TASK-107.11's unchanged local and repository-managed Lima/SSH harness passed all five groups; Actions run 34915583460 built and natively smoke-tested all four release targets on matching runners; `cmd/worklease-doc-test` validates the aligned public guides. The capability remains experimental and no publication, tag, push, or release is authorized by this amendment.

## 18. Internal API sketches

These names locate responsibilities; signatures must carry the fields required by the amended contract. Prefer existing typed requests/results over a generic backend framework. The shipped remote implementation extends these boundaries without introducing a generic backend registry; extract consumer-specific interfaces only for demonstrated needs.
...
## 20. Experimental remote authority boundary (TASK-107 shipped)

### 20.1 Scope and deployment

The default remains the local POSIX SQLite authority. TASK-107 ships an opt-in, self-hosted remote authority as an experimental capability: the HTTP client/server, profile and invitation enrollment, installation roles, hosted administration, and local stdio MCP client are implemented and bounded by the measured evidence below. Historical local behavior, including D7/D8 and local `--no-handle`, remains unchanged apart from the shared schema migration. The standard binary permanently opens no listener and performs no network request unless the user selects a remote profile, explicitly invokes remote profile management, or runs `worklease serve`; local reads remain setup-free.

The remote capability serves the existing Go `lease.Service` over authenticated HTTPS from one always-on host, one persistent SQLite volume, and one namespace per process. One serialized SQLite writer remains the namespace throughput boundary. A deployment-owned configuration file lists delimiter-terminated portable prefixes and TTL/hold bounds; known host-local prefixes remain forbidden, and configuration changes take effect only after stop/start. A trusted reverse proxy may terminate TLS but sees bearer secrets; proxy identity headers grant no authority. Optional asynchronous object backup is disaster recovery, not high availability. Do not add a backend registry, remote process runner, provisioning system, or second claim implementation. `docs/remote-claim-authority.md` records the detailed chosen design, rejected alternatives, and follow-up triggers.

### 20.2 Authority, resources, and schema

Create immutable random `authorityId` and `restoreId` values in authority metadata. A path, URL, or DSN is a locator, not identity. Ordinary restart preserves both values; every restore, import, or recreation that may lose acknowledged writes generates a new random `restoreId`. Do not copy an active database into another writable authority with the same identity.

Resources contend by exact bytes within one authority namespace, and a 1-to-32-resource claim stays atomic. Local `path:`, `backlog-md:`, and `markdown:` identities are always rejected remotely. The initial remote slice admits only configured portable prefixes such as `github:` and `coordination:generic:`. Persist each claim's admitted maximum TTL and absolute hold deadline, enforce them on every expiry extension and same-host transfer, and never rewrite hashed intent by clamping an over-bound request. Repository enrollment, alias catalogs, cross-host transfer, sharding, and multi-namespace serving remain deferred.

The remote-capable store uses a shared schema version 2 with a one-way migration. It adds installation and invite records, recovery-mode and reopening-attestation records, restore and revoked epoch-end reasons, per-claim admitted limits, trusted installation attribution, incarnation provenance, and bootstrap recovery records. A local schema-1 home keeps its historical behavior until migration; an older binary refuses version 2 with `schema-unsupported`.

### 20.3 Protocol, authentication, and client recovery

The initial remote wire and command surface is frozen by reference to [Remote Authority Protocol V1](../remote-authority/doc-4%20-%20Remote-Authority-Protocol-V1.md). That specification exclusively owns `worklease-http/1`, exact routes and flags, wire fields and typed extensions, envelopes, bounds, cancellation, error-to-HTTP mapping, serialized authorization/replay/admission order, and the operation exposure matrix. The shipped TASK-107 implementation conforms to it; changing that surface requires this section 15 procedure. Independent hosted-lock work may proceed on its own dependency path. Map only allowed typed service fields, never client-selected compatibility inputs such as `LegacyRequestHash`. Every Worklease application response carries freshly sampled `authorityTime` and current `restoreId`; cursors, receipts, handles, and replay records retain authority and incarnation provenance. Existing key-derivation and instruction MCP tools remain client-local even when a remote profile is selected; they may derive a host-local key, while remote admission rejects host-local prefixes. Authorized structured requests and results may carry resource keys. `BeginOperation` and completion may transmit bounded private argv intent or receipt data, but the remote server never executes it.

Installation credentials are separate from claim credentials. Enrollment uses short-lived one-shot invites and client-generated installation credentials saved durably before dispatch; the authority stores only hashes. Explicit `read`, `write`, and `admin` roles have the grants defined in `docs/remote-claim-authority.md`. There is no OAuth server, browser login, or session. A bounded public identity metadata route returns only `authorityId`, `restoreId`, protocol version, and `authorityTime`; health is separate. Enrollment is invite-authenticated, and every other API is installation-authenticated. Metadata, health, and enrollment are bounded and rate-limited.

For an installation-authenticated request, authenticate the current installation, check revocation and role, validate `authorityId` and immutable `expectedRestoreId`, authenticate the target epoch where required, resolve exact replay, then apply new-admission recovery, prefix, and bound checks. Recheck mutable authorization and namespace state in the same serialized mutation transaction. Concurrent revocation and mutation follow their serialization order; revocation is not retroactive. Unknown and retained-revoked installations return `authentication-required` and `installation-revoked` before incarnation errors. A correctly authenticated request with the wrong incarnation returns `authority-restored` in a fresh current-incarnation envelope and performs no replay or mutation. Enrollment validates its invite, authority, and expected incarnation before burning the invite or inserting an installation.

Built-in remote clients durably save enumerable exact pending state before every mutation, including enrollment, administration, and `--no-handle`. Save exact normalized inputs, original authority/incarnation, generated request/claim/operation IDs, request hash, full resources, retry deadline, `requestNotAfter`, and dispatch intent before sending; keep secrets in existing credential and handle stores. Claim operations reuse handle pending state, while bounded request-scoped profile records cover secondary and handleless mutations. A `BeginOperation` acknowledgment does not clear the parent guarded-effect record, and renewal, completion, or reconciliation cannot overwrite it. Resolve a record only after confirmed terminal or reconciliation, or definitive proof that the original dispatch did not commit. Timeout, authentication, revocation, incarnation change, age, replay expiry, GC, refresh, and rotation do not clear or rebind uncertain state. Custom remote clients provide equivalent persistence. A full completed-history journal and optional successful-history export remain deferred.

### 20.4 Restore, recovery, and administration

Restore ends retained active claims as `restored`, preserves retained started operations as unresolved, revokes retained installations and invites, and enters namespace recovery mode. Recovery mode refuses ordinary admission and new `BeginOperation` calls, but resolves retained replay first and permits ordinary claims that cover the full transitive unresolved resource closure within the 32-resource limit. Such claims may reconcile and may start new effects only after reopening. Recovery mode cannot stop work already dispatched.

Record the actual selected durable restore cutoff and the lost-history interval through old-authority cessation, with delayed provider work accounted for separately. Measured replication lag does not bound an older selected backup or a monitoring gap; an unknown cutoff remains an unknown bound and never waives required coverage. Reopening requires an independent inventory of every active, retired, and ephemeral installation that may have acted since the cutoff, each enumerable unresolved request set or equivalent independently verified coverage, outcomes for retained unresolved and otherwise known in-flight work, and namespace-wide executor and asynchronous-provider cessation evidence. A retained start with lost completion requires reconciliation even if client pending was cleared. A fully lost start and completion has no import target; a quiet completed operation may remain unenumerated only when independent coverage establishes no residual effect across the namespace. Never redispatch an old effect from a missing row or completion acknowledgment. Missing coverage keeps recovery mode set indefinitely. Worklease validates evidence structure and retained-row reconciliation, not external truth. A durable retained receipt or journal is a follow-up when drills cannot meet the selected recovery target or establish required coverage; it helps enumeration and never proves cessation. Recovery import remains deferred and cannot discover missing history.

The authenticated admin service owns invite issuance, installation and claim revocation, bounded private inspection, GC apply, and a reopening method that serializes the retained-unresolved check, bounded private attestation, and atomic reopen. Ordinary invite issuance and enrollment are closed during recovery; the sole exception redeems a current-incarnation offline bootstrap admin invite. Initialization, restore, bootstrap reissue, and retirement are offline commands authorized by local OS/home permissions, run with the server stopped, and hold the hosted lock. Authentication and domain changes compose in one serialized database transaction, but that transaction cannot be atomic with the private secret file. Use crash-recoverable ordering so the secret is durable before its grant is usable, incomplete state refuses serving, and plaintext secrets never enter the database. Bootstrap reissue preserves authority incarnation and claims and may append an administrative audit event. Retirement is not an HTTP admin route.

### 20.5 Coordination guarantee and deferred work

Remote exec supervision remains client-local coordination. Renew at half TTL; at three quarters without confirmation stop new guarded work and report uncertainty. Begin bounded process-group termination no later than conservative last-known expiry, or immediately after confirmed ownership, authentication, or incarnation loss. A late start acknowledgment after conservative expiry never dispatches the effect. Completion after child exit does not fence asynchronous provider work or escaped descendants, so recovery still requires outcome and cessation evidence. Local supervision behavior is unchanged.

A hosted-home marker makes direct local authority mutations refuse before opening the database for writes and directs callers to the remote profile. Only `serve` and explicit offline hosted administration, maintenance, and migration entry points may write, under the stable process-lifetime hosted lock and remote invariants. Ordinary local homes keep their existing concurrency. The advisory lock protects one stable inode on a single-host filesystem; it does not protect writable clones.

Do not add a fencing counter until an enforcing consumer exists. A future provider fence must increase across epochs per resource, survive retention and restore, and remain scoped to one authority namespace; claim revision and event sequence are not fences. Remote replacement stays disabled without a separately specified enforcing adapter. Cross-host transfer, recovery import, a completed-history journal, admission backpressure, browser login, control plane, managed Postgres, multi-replica service, and public hosting remain separate follow-ups with their recorded triggers. Configured remote failure never falls back to local ownership.
