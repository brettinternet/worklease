...
| View of 50,000 summaries across sources | Navigable, within a provisional 512 MiB RSS budget excluding provider processes, which are reported separately. The original 256 MiB provisional target fails: the prior unprepared refresh measured p95 peak RSS 450,887,680 bytes (430 MiB); the current worker-prepared path measured p95 362,987,520 bytes (346 MiB) across 20 standalone D22 processes. The 512 MiB revision leaves room above these measured peaks, but full provider/index concurrency and terminal paint remain unverified; do not call this budget met yet. |
| 100,000-summary browsing stress | Remains navigable with bounded windows. The measured failure boundary is full-snapshot responsiveness, not warm navigation: the current worker-prepared 100,000-summary path measured standalone p95 refresh+View 83.66 ms and peak RSS 716,914,688 bytes (684 MiB), versus p95 warm navigation 2.33 ms. The earlier unprepared path measured p95 refresh+View 878 ms and 855,064,576 bytes (816 MiB). Even with worker preparation, 100,000-summary refresh exceeds the concurrent-refresh 50 ms input-loop budget; this is not evidence of a safe terminal experience. |
