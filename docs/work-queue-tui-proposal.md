...
Initial declarations from §3 evidence; Linear incorporates its synthetic TEST/PDEV probe, and Beads its embedded-Dolt CLI probe. Among the new S8 candidates, only Beads is shipped:
...
| State | Configured statuses; terminal mapping from caller config | Open or closed with `stateReason`; Projects v2 mapping accepted but unimplemented (D32) | Raw workflow types backlog/unstarted/started/completed/duplicate/canceled; configured state-ID transitions required | Raw open/in_progress/blocked/deferred/closed; configured state transitions via `bd statuses --json` |
...
| Authentication | OS file access | `gh` helper for an explicit host and account | User-supplied token resolved via Keychain for probe; viewer/org IDs observed; helper bounds and principal-gate are TASK-142.2 | Local CLI and explicit checkout; effects require consent |
