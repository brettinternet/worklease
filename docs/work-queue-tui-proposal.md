...
Initial declarations from §3 evidence; Linear incorporates its synthetic TEST/PDEV probe, while other S8 columns remain unprobed proposals. None are shipped capabilities:
...
| State | Configured statuses; terminal mapping from caller config | Open or closed with `stateReason`; Projects v2 mapping accepted but unimplemented (D32) | Raw workflow types backlog/unstarted/started/completed/duplicate/canceled; configured state-ID transitions required | Status mapping and transitions unknown |
