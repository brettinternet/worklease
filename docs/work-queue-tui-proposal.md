...
Initial declarations from §3 evidence; Linear incorporates its synthetic TEST/PDEV probe and optional GitHub Projects v2 status uses the TASK-140 synthetic probe. Beads remains unprobed. Entries describe observed semantics, not blanket authorization:

| Group | Backlog.md 1.52 | GitHub Issues (github.com) | Linear (S8, synthetic probe only) | Beads (S8, pending probe) |
| --- | --- | --- | --- | --- |
| Identity | Task ID in one explicit checkout; host-local key or D12 binding; renumbered by duplicate repair | `owner/repo` and number; node IDs retained; rename and transfer detected | Organization ID + issue UUID stable across TEST→PDEV→TEST move; aliases mutable | Proposed explicit checkout and portable `generic` binding; ID stability unknown |
| Discovery | Complete list in one call; no cursor; exact observed total | GraphQL cursor pages of 100; observed `totalCount`, not a multi-page snapshot; pull requests excluded | Relay `first` ≤250, `updatedAt.gte`, `orderBy:updatedAt`; moving rows between pages skip issues, so full/overlapping reconciliation is required | CLI JSON schema and scale cost unknown |
| Dependencies | Intra-project edges per task view; closure per item; provider-reported `isReady` | `blockedBy`/`blocking` per item with totals; cross-repository references; sub-issues are hierarchy only | `blocks` uses `relations`/`inverseRelations`; relation add advanced only inverse endpoint; removal advanced both, so independently reconcile edges | Typed-edge output, direction and completeness unknown |
| State | Configured statuses; terminal mapping from caller config | Open or closed with `stateReason`; optional bound Projects v2 single-select option ID maps advisory workflow state with raw name and explicit conflicts, not issue completion (D32) | Raw workflow types backlog/unstarted/started/completed/duplicate/canceled; configured state-ID transitions required | Status mapping and transitions unknown |
| Progress | Append notes or comment; criterion-index edits exist but queue writes are initially disabled (§8) | Append comment | Comment marker survives create/read-back; issue `updatedAt` did not advance | Note/comment marker support unknown |
| Assignment | Multiple, replace-all only | Multiple, add and remove endpoints | One `assignee` / `assigneeId` replacement field; tested assign-to-viewer | Cardinality and semantics unknown |
| Native claims | Not exposed | Not exposed | No §9-admissible native lease established; disabled, Worklease-only | Unknown; in-progress/assignment are not claims |
| Mutation | Unconditional CLI edits | Unconditional; no conditional unsafe methods | Synthetic state/assignee, relation, comment, archive/trash/delete tested; production writes disabled pending recovery verification | Unknown; probe Git/daemon effects before writes |
| Synchronization | Watch stream or filesystem invalidation; minute timestamps | Issue `since` filter and conditional-GET polling; bound project items need independent complete pages because status-only updates do not advance issue `updatedAt`; no client webhooks | `updatedAt.gte` works but relation adds, comments and archive may not update source endpoint; independently reconcile relations and archived state | Bulk output or invalidation signals unknown |
| Effects | Optional Git fetch, commit, and hooks per project config | Notifications to watchers | Synthetic edits visible through GraphQL; external notification effects untested | Git/daemon/network side effects unknown |
