...
| View of 50,000 summaries across sources | Navigable, within a provisional 512 MiB RSS budget excluding provider processes, which are reported separately. The original 256 MiB provisional target fails: a standalone five-source Go TUI model with 50,000 summaries measured p95 peak RSS 450,887,680 bytes (430 MiB) on D22 over 20 runs. The 512 MiB revision leaves room above this measured peak, but full provider/index concurrency and terminal paint remain unverified; do not call this budget met yet. |
| 100,000-summary browsing stress | Remains navigable with bounded windows. The measured failure boundary is full-snapshot responsiveness, not warm navigation: standalone p95 refresh+View 878 ms, peak RSS 855,064,576 bytes (816 MiB), versus p95 warm navigation 2.33 ms in the Go benchmark. Full-refresh work on the input event loop violates the concurrent-refresh 50 ms budget already at 10,000 summaries; this is a known prototype exit failure, not evidence of a safe 100,000-summary terminal experience. |
