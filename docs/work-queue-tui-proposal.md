...
| Group | Backlog.md 1.52 | GitHub Issues (github.com) | Linear (S8, pending probe) | Beads 1.3.0 (probed) |
| --- | --- | --- | --- | --- |
| Identity | Task ID in one explicit checkout; host-local key or D12 binding; renumbered by duplicate repair | `owner/repo` and number; node IDs retained; rename and transfer detected | Proposed organization ID + issue UUID; move behavior unverified | Explicit embedded-Dolt checkout; hash or explicit IDs; portable `generic` binding, duplicate/disappearance guard |
| Discovery | Complete list in one call; no cursor; exact observed total | GraphQL cursor pages of 100; observed `totalCount`, not a multi-page snapshot; pull requests excluded | Relay pagination limits/order and filters unknown | Complete JSON bulk list, no cursor; 10k list ~1.4 s, 327 MiB RSS |
| Dependencies | Intra-project edges per task view; closure per item; provider-reported `isReady` | `blockedBy`/`blocking` per item with totals; cross-repository references; sub-issues are hierarchy only | Proposed blocking edges; direction, completeness and change signals unknown | Complete typed edges in bulk; `blocks` hard; other types informational; provider-ready from separate bulk list |
| State | Configured statuses; terminal mapping from caller config | Open or closed with `stateReason`; Projects v2 mapping accepted but unimplemented (D32) | Workflow state types and transitions unknown | Raw open/in_progress/blocked/deferred/closed; configured state transitions via `bd statuses --json` |
| Progress | Append notes or comment; criterion-index edits exist but queue writes are initially disabled (§8) | Append comment | Comment marker round-trip unknown | Marked append-only comment; comments JSON yields id, author and text |
| Assignment | Multiple, replace-all only | Multiple, add and remove endpoints | Proposed single assignee; replacement semantics unverified | Single assignee; `bd update --assignee` replaces value |
| Native claims | Not exposed | Not exposed | Unknown; study only if §9 evidence warrants | Not exposed to Worklease; in-progress/assignment are advisory |
| Mutation | Unconditional CLI edits | Unconditional; no conditional unsafe methods | Unknown; no writes before probe/recovery verification | Non-conditional CLI edit; Dolt commit, comment receipt and read-back through recovery pipeline |
| Synchronization | Watch stream or filesystem invalidation; minute timestamps | `since` filter, conditional-GET polling; no client webhooks | `updatedAt` and relation-reconciliation behavior unknown | Complete bulk list and periodic polling; no per-issue refresh subprocess |
| Effects | Optional Git fetch, commit, and hooks per project config | Notifications to watchers | Unknown | Embedded Dolt; local commit on write, optional Git-tracked JSONL auto-export, no Git commit/hooks; remote config requires consent |
