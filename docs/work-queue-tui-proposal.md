...
| D10 | Sources, views, identity mappings, and launch actions live in one owner-private user file, `$XDG_CONFIG_HOME/worklease/queue.yaml`, following the profiles/bindings trust model. v1 reads no repository-provided queue configuration. `queue init` preflights the selected adapter and writes this owner-private file from detected provider facts on explicit invocation; `--dry-run` previews without writing. Backlog.md `me` falls back to the OS login, and `--allow-git-network` explicitly consents to project Git network effects. | Existing pattern |
...
| D14 | The GitHub adapter speaks HTTPS (GraphQL and REST) directly. It uses `gh auth token --hostname H --user U` as a credential helper for an explicitly configured account, verifies the principal, and sends requests serially per host/account within each client process. Honor REST and GraphQL rate-limit headers and GraphQL `rateLimit` fields; this probe does not establish a cross-process quota coordinator. Optional Projects v2 status reads require `read:project`; writes require separately approved `project` scope. | Live github.com evidence; GHES unprobed |
...
| D16 | External adapters use the stable v1 JSON-RPC 2.0 stdio protocol, with a manifest, bounded conformance check (`queue adapter check --json`), explicit source configuration (`queue init --adapter external --executable PATH`) and digest-bound approval. The host delivers optional helper-resolved credentials over a narrow source-scoped channel; recoverable writes use receipts. Resource policies stay static built-ins (`generic` for external sources). No SDK or generic MCP-bridge adapter ships. | Shipped S7 + TASK-143 |
| D17 | Other extension seams are bounded credential helpers, declarative views, filters, status maps, keymaps, and user-configured launch actions. No executable policy plugins, widgets, lifecycle hooks, storage plugins, event bus, or SDKs. Hooks and plugins remain out of scope, not substitutes for adapter receipts or claim keys. | User + shipped design |
...
| D29 | Accept a built-in Linear Go adapter, using its GraphQL HTTPS API and a user-configured credential-helper argv (token only in memory). Verify `viewer` on credential change and before every write. Use the existing static `linear` resource policy with the organization ID as source and issue UUID as item; team and mutable issue identifier are locators/display only. Serialize requests per organization/account within each client, respecting request and complexity limits. Bound Relay pages at 250; use overlapping/reconciled scans, not a single moving `updatedAt` cursor, and reconcile relations independently of endpoint timestamps. Treat archive/trash and permission loss as distinct from deletion; no provider-native claim authority is admitted. Writes remain disabled until the read/sync/claim and recovery subtasks establish their capabilities. | User request and TASK-142.1 live TEST/PDEV probe on 2026-09-25 (§3); permission loss and concurrent multi-client quotas unprobed |
...
| D31 | The built-in Beads adapter pins `bd` 1.3.0, uses JSON CLI reads and documented update/comment flags on one explicit embedded-Dolt checkout. A complete bulk list provides typed edges; `blocks` is hard, hierarchy and discovery are informational. `bd --readonly --sandbox` prevents read mutations and auto-push; writes force a local Dolt commit with remote auto-push disabled. A configured Dolt remote requires `allowGitNetwork` consent. Auto-export may alter Git-tracked JSONL but no queue write creates a Git commit or runs Git hooks. Portable claims require an explicit `generic` source binding and identity confirmation; Beads assignment and in-progress state remain advisory. | Disposable Beads 1.3.0 probe on 2026-09-25; TASK-141 |
| D32 | Accept an optional GitHub Projects v2 status mapping in the existing GitHub adapter, bound to one explicit owner/number/node-ID project and single-select field/option IDs. Project field updates do not advance issue `updatedAt`; sync project items independently. Status updates are unconditional and need exact item/field read-back through §8 recovery. Existing GitHub claim keys and issue completion semantics remain authoritative. | User request and synthetic github.com probe on 2026-09-25; TASK-140 supersedes §17 S6 deferral |
...
### Linear (TASK-142.1, 2026-09-25)

Live GraphQL HTTPS probe with a Keychain-held API key, against the user-approved TEST and PDEV teams only. `viewer.id=72088203-6bc7-4a63-a71b-22048b88da64`, `viewer.organization.id=0bfebf80-70af-4eca-9e39-2029a01f5b77`, TEST team `d19193f6-0501-485b-93af-65e829c2039d`, PDEV team `db1de176-9bc6-4470-87cb-2b8005699826`. Only two new synthetic TEST issues (`TEST-1` UUID `075a1740-eeda-4b85-be0b-39755abf4c8c`, `TEST-2` UUID `9f3b2707-b6d8-456d-9079-32f60cd33474`) were mutated. Both were permanently deleted by `issueDelete(permanentlyDelete:true)` with `success:true`; a subsequent TEST `issues(includeArchived:true)` returned zero nodes. No pre-existing issue was mutated or its content read.

| Observation | Consequence |
| --- | --- |
| Moving TEST-1 to PDEV changed its identifier to `PDEV-5` while retaining UUID `075a…`; moving it back restored `TEST-1` and populated `previousIdentifiers` (including a historical team-prefix alias). | Bind the claim source to organization ID and item to UUID. Team and identifier are mutable locators; never bind to an alias or treat alias history as unique identity. |
| `team.issues(first:1, orderBy:updatedAt)` returned Relay `pageInfo.hasNextPage/endCursor`; page two returned the other synthetic issue. `filter:{updatedAt:{gte:…}}` worked. `first:251` returned `INVALID_INPUT` (`first must not be greater than 250`). After page one returned TEST-2, updating TEST-1 moved it ahead; page two after TEST-2 returned **zero** although TEST-1 remained in the team. | Cap pages at 250, specify ordering, overlap/reconcile whole scans and never advance a complete-source watermark or prove deletion from a moving cursor scan. |
| Creating `blocks` from TEST-1 to TEST-2 appeared in TEST-1 `relations` and TEST-2 `inverseRelations`. On add, only TEST-2 `updatedAt` advanced; on removal, both advanced. `IssueRelationType` also lists `duplicate`, `related`, and `similar`. | Either endpoint's `updatedAt` is **not** a reliable relation-add invalidation signal. Page both relation directions; reconcile edges independently. Treat only directional blocking as hard, other types as informational. |
| TEST workflow states exposed `backlog`, `unstarted`, `started`, `completed`, `duplicate`, and `canceled` types (with raw state ID/name). Updating TEST-2 to `started` plus `assigneeId=viewer.id` read back one `assignee` object. The issue update input has a single `assigneeId`, not an add-assignee list. | Map raw state type and configured transitions explicitly; assignment is single-slot replacement, not an additive operation. Require explicit confirmation before replacing another assignee. No status/assignment is a lease. |
| `commentCreate` with `<!-- worklease-op:1421-probe-20260925 -->` returned the body verbatim; a separate `issue.comments` read returned the same marker/body. Posting the comment did not advance the issue's `updatedAt`. | Marker-based recovery can read comments on the exact issue, but a comment-only update cannot rely on issue `updatedAt`; marker alone is not idempotency or proof of actor/payload. |
| `issueArchive` set `archivedAt` without advancing `updatedAt`; default `team.issues` omitted the archived issue, `includeArchived:true` included it, and direct UUID lookup returned it. `issueArchive(trash:true)` set `trashed:true`; even `includeArchived:true` included it. Permanent deletion yielded `INPUT_ERROR` on direct lookup and removed both from the complete test-team listing. | Reconcile archived/trashed flags rather than equate absence or an unchanged cursor with deletion. A lookup error may also be permission loss; never infer deletion from it alone. |
| Responses carried `x-complexity`, `x-ratelimit-complexity-limit/remaining/reset` (3,000,000), and `x-ratelimit-requests-limit/remaining/reset` (2,500); measured simple requests cost 2–253 complexity points. | Budget both quotas per organization/account and honor reset/retry responses; the probe did not load-test concurrency or force rate limiting. |

**Unverified:** Access revocation/permission-loss visibility was not tested because no second restricted principal was provided and changing existing team membership was outside the approved synthetic-issue scope. Treat inaccessible, missing, or GraphQL `INPUT_ERROR` outcomes as unknown, never deletion or write permission. No native atomic acquire/renew/expiry/fencing API satisfying §9 was established; use Worklease coordination only, with native authority disabled. No cross-team partial-access, 250-row boundary at scale, concurrent writers, or read-after-write lag was established. This probe does not authorize production writes; TASK-142.2–142.7 must prove the remaining paths.

### Beads 1.3.0 (S8)

Pinned release `v1.3.0` (`bd version 1.3.0`, Darwin arm64 archive SHA-256 `7cc77367d0b84c50243a1108bc1f73648699211257d414b917540bf868e6bb85`) on the D22 Apple M1 Max, 32 GiB. The [disposable fixture and benchmark](../scripts/beads-queue-benchmark.py) uses `bd init` and `bd import --json` in empty OS-temp Git repositories, with ten-item blocking chains. Each measurement below is one process; wall times are three samples (not p95 bounds); RSS uses a separate `/usr/bin/time -l` sample. The 10,000-row import took 582 s and is fixture setup, not refresh cost.

| Issues | `list --all --limit 0 --brief --json` s | `show probe-00002 --json` s | List peak RSS MiB |
| ---: | --- | --- | ---: |
| 102 | 0.380, 0.361, 0.385 | 0.366, 0.359, 0.350 | 134 |
| 1,000 | 0.467, 0.418, 0.430 | 0.384, 0.398, 0.385 | 145 |
| 10,000 | 1.431, 1.452, 1.406 | 0.972, 0.970, 0.952 | 327 |

`bd --json version` returns `{version, commit, build, schema_version}`. `list --all --limit 0 --brief --json` is an array of id, title, raw status, priority, assignee when present, updated_at, dependency_count, and **complete `dependencies`** (each `{issue_id,depends_on_id,type,created_at,created_by,metadata}`); `show --json` is a one-element array with description, notes, revision, and nested dependencies using a different shape (`dependency_type`). `list --ready --all --limit 0 --brief --json` provides provider-reported readiness. An omitted dependency array is valid only with zero dependency_count; a partial or invalid list never proves deletion. Bulk output removes the Backlog.md per-item edge cost. `blocks` points from dependent `issue_id` to prerequisite `depends_on_id`; parent-child, related, discovered-from and other observed relation types are not hard edges (D27). Poll a complete bulk list, including readiness, rather than one process per issue.

The earlier JSONL/daemon premise was wrong for 1.3.0: embedded Dolt is the default and starts **no daemon** (`bd dolt status --json` reported `mode: embedded`, `server_running: false`). `bd init` committed `.beads` configuration to Git once, but `bd create`, `bd update`, comments, and readonly list/show did not create Git commits or run hooks; normal writes created **Dolt commits** (`bd vc status --json` changed commit, and `bd history --json` named the new commit). Default `export.auto=false` left Git clean; when set true, a write produced an untracked `.beads/issues.jsonl` export, whereas a readonly list did not change Git state. A separate unrelated staged file remained staged after queue-equivalent Beads writes. `--sandbox` disables Dolt auto-push; `--readonly` blocks read writes. A disposable `file://` Dolt remote named `probe` with `sync.remote=probe` pointing at a nonexistent destination did not stop `bd --readonly --sandbox list` or `bd --sandbox create`; neither auto-pulled nor auto-pushed to that missing remote. This is not a proof for every remote transport or configuration: a configured `sync.remote` is conservatively treated as possible network access and requires consent, while the local embedded/no-remote path has no remote destination to contact. Server mode is not supported by this adapter. Git hooks are not part of the Beads write path; Dolt commits are disclosed separately from Git commits. Auto-export is throttled and disclosed as a possible Git worktree change, never a verified Git commit.

`bd init` generated `probe-<hash>` IDs; a clone containing only Git-tracked `.beads` config bootstrapped an **empty** Dolt database without a Dolt remote, and generated a different hash ID (`probe-m5d` versus `probe-g51`). Thus Git cloning alone does not synchronize issue data. `bd create --id probe-m5d` succeeded in that independent clone while the original checkout already had a different `probe-m5d` issue. JSONL import also accepts explicit IDs. Hash generation does not prove global uniqueness or prevent renames. Confirm the same source binding and actual Dolt sync domain across workers, detect duplicates from a complete list, and require migration on disappearance of a confirmed ID. Beads `in_progress`, assignment, and `bd update --claim` are provider-local occupancy, not a Worklease claim.

### GitHub Projects v2 status (TASK-140, 2026-09-25)

User-approved probe on two **private synthetic** user-owned projects, #5 (`PVT_kwHOAT55tM4BksRF`) and #6 (`PVT_kwHOAT55tM4BksRN`), with a synthetic issue in `brettinternet/worklease` (#6), one draft, and an existing closed PR (#2) added as a project item without changing the PR. Cleanup receipts are tracked in TASK-140. No unrelated project was changed. GHES and permission revocation remain unprobed.

| Observation | Consequence |
| --- | --- |
| `gh project list` initially rejected a token without `read:project`; after requesting only `read:project`, it listed user projects, and `field-list` discovered the default Status field with option IDs. Project creation and `item-edit` required an additional `project` scope, separately approved. A custom single-select Workflow Probe field (`PVTSSF_lAHOAT55tM4BksRFzhjb-Hs`) exposed option IDs for In Progress, Blocked, In Review, and Done. | Setup requests read-only scope first. Bind owner, number, immutable project and field IDs, and option IDs; rediscover and fail closed on loss or rename. Enable writes only with separately approved scope. |
| `project.items(first:2)` reported `totalCount=3`, `hasNextPage=true` and an end cursor; the next page contained the issue with `hasNextPage=false`. Items included `DraftIssue`, `PullRequest`, and `Issue` with distinct GraphQL types. The same issue in projects #5 and #6 had two distinct project-item IDs. | Page the *bound* project's items completely before deriving mapping; ignore drafts/PRs and other project memberships when mapping repository issues. Interrupted scans do not prove a complete mapping. |
| Changing the custom field on project #5 issue item `PVTI_lAHOAT55tM4BksRFzg8zY6Y` to In Progress advanced item `updatedAt` from 17:29:48Z to 17:30:13Z; issue `updatedAt` remained 17:29:33Z. The exact item read returned the configured field ID, option ID `6760e0a3` and name. | Issue `updatedAt` cannot detect project-only state changes. Refresh project items independently; after an unconditional write verify the exact project item and field/option, never retry solely because the response was lost. |
| Field discovery, item pages, and exact-item read queries each reported GraphQL `rateLimit.cost=1` in this tiny fixture (remaining quota fell by one per query). | Charge every page and verification read against the shared account GraphQL budget; this does not establish cost at scale or across clients. |

The project status remains advisory workflow state. The issue's open/closed state and completion reason retain their own evidence; Done on an open issue is a visible conflict, not proof of completion. Existing GitHub claim keys are unaffected (D24, D27).
...
#### Shipping terminal verification (TASK-135, 2026-09-23)

On the D22 Apple M1 Max / 32 GiB, the shipping `worklease queue` displayed a disposable 10,000-row Backlog.md fixture. The first task's title contained CJK, emoji, CSI color and OSC window-title sequences; its visible title was `仕事 🙂 BAD`, with controls stripped. Terminal versions: Terminal.app 2.15, iTerm2 3.7.2, Ghostty 1.3.1, tmux 3.7c, and Herdr 0.9.1. In an interactive session the user confirmed, in Terminal.app, iTerm2, Ghostty, and a Herdr pane, 10,000 displayed rows, aligned and sanitized first-row text, the list/detail change on resizing across 100 columns, `NO_COLOR=1`, and a usable terminal after `q`. Independently, tmux captures at 120×35 and 80×25 showed 10,000 rows, the sanitized title and the side-by-side/detail-to-single-pane switch; a `NO_COLOR=1` capture had no styling escapes, and `q` closed the tmux session. No product defects were observed.

The initial fresh-index `source-resolve-failed` diagnostic was a fixture environment problem: the `backlog` mise shim had no selected version outside the repository. Adding the installed Backlog.md 1.52.0 binary to the disposable launcher's `PATH` resolved it. The fixture and launcher remain outside the repository. End-to-end input-to-paint latency is still owned by TASK-129.6, not this verification.
...
**Beads effects (D31).** Only a configured embedded-Dolt checkout is admitted. Local-only reads use `--readonly --sandbox` and require an empty `sync.remote`; a configured remote requires `allowGitNetwork: true` consent before reads even though the adapter does not invoke `bd sync`. Writes use `--sandbox --dolt-auto-commit=on`: they create a local Dolt commit, disable auto-push, do not invoke Git hooks or create a Git commit, and may update Git-tracked `.beads/issues.jsonl` when `export.auto=true`. Preview discloses the Dolt commit and optional export. Read-back verifies item fields or the exact comment/marker and observes the resulting Dolt commit before checkpointing. Remote server mode is unsupported.

**Backlog.md Git effects.** With `remote_operations` or `check_active_branches` enabled, a Backlog.md read may contact Git remotes. The source is then treated as remote access and needs the same explicit consent as GitHub. With `auto_commit` enabled, a write creates a task-file commit and runs hooks unless `bypass_git_hooks` is set. TASK-126.6 verified in a scratch project that unrelated staged changes stay staged; the Backlog.md write adapter also tests that behavior with auto-commit on and off. Its preview discloses the commit and hook effects, and its read-back checks the task-file commit before checkpointing. Status, progress, and assignment edits use only validated Backlog CLI argv through the recoverable write pipeline; criterion-index writes remain unavailable. The queue UI still needs its separate mutation preview and action wiring (TASK-132.4/132.5).
...
Initial declarations from §3 evidence; Linear incorporates its synthetic TEST/PDEV probe, Beads its embedded-Dolt CLI probe, and optional GitHub Projects v2 status the TASK-140 synthetic probe. Entries describe observed semantics, not blanket authorization:

| Group | Backlog.md 1.52 | GitHub Issues (github.com) | Linear (S8, synthetic probe only) | Beads 1.3.0 (probed) |
| --- | --- | --- | --- | --- |
| Identity | Task ID in one explicit checkout; host-local key or D12 binding; renumbered by duplicate repair | `owner/repo` and number; node IDs retained; rename and transfer detected | Organization ID + issue UUID stable across TEST→PDEV→TEST move; aliases mutable | Explicit embedded-Dolt checkout; hash or explicit IDs; portable `generic` binding, duplicate/disappearance guard |
| Discovery | Complete list in one call; no cursor; exact observed total | GraphQL cursor pages of 100; observed `totalCount`, not a multi-page snapshot; pull requests excluded | Relay `first` ≤250, `updatedAt.gte`, `orderBy:updatedAt`; moving rows between pages skip issues, so full/overlapping reconciliation is required | Complete JSON bulk list, no cursor; 10k list ~1.4 s, 327 MiB RSS |
| Dependencies | Intra-project edges per task view; closure per item; provider-reported `isReady` | `blockedBy`/`blocking` per item with totals; cross-repository references; sub-issues are hierarchy only | `blocks` uses `relations`/`inverseRelations`; relation add advanced only inverse endpoint; removal advanced both, so independently reconcile edges | Complete typed edges in bulk; `blocks` hard; other types informational; provider-ready from separate bulk list |
| State | Configured statuses; terminal mapping from caller config | Open or closed with `stateReason`; optional bound Projects v2 single-select option ID maps advisory workflow state with raw name and explicit conflicts, not issue completion (D32) | Raw workflow types backlog/unstarted/started/completed/duplicate/canceled; configured state-ID transitions required | Raw open/in_progress/blocked/deferred/closed; configured state transitions via `bd statuses --json` |
| Progress | Append notes or comment; criterion-index edits exist but queue writes are initially disabled (§8) | Append comment | Comment marker survives create/read-back; issue `updatedAt` did not advance | Marked append-only comment; comments JSON yields id, author and text |
| Assignment | Multiple, replace-all only | Multiple, add and remove endpoints | One `assignee` / `assigneeId` replacement field; tested assign-to-viewer | Single assignee; `bd update --assignee` replaces value |
| Native claims | Not exposed | Not exposed | No §9-admissible native lease established; disabled, Worklease-only | Not exposed to Worklease; in-progress/assignment are advisory |
| Mutation | Unconditional CLI edits | Unconditional; no conditional unsafe methods | Synthetic state/assignee, relation, comment, archive/trash/delete tested; production writes disabled pending recovery verification | Non-conditional CLI edit; Dolt commit, comment receipt and read-back through recovery pipeline |
| Synchronization | Watch stream or filesystem invalidation; minute timestamps | Issue `since` filter and conditional-GET polling; bound project items need independent complete pages because status-only updates do not advance issue `updatedAt`; no client webhooks | `updatedAt.gte` works but relation adds, comments and archive may not update source endpoint; independently reconcile relations and archived state | Complete bulk list and periodic polling; no per-issue refresh subprocess |
| Effects | Optional Git fetch, commit, and hooks per project config | Notifications to watchers | Synthetic edits visible through GraphQL; external notification effects untested | Embedded Dolt; local commit on write, optional Git-tracked JSONL auto-export, no Git commit/hooks; remote config requires consent |
| Authentication | OS file access | `gh` helper for an explicit host and account; `read:project` for optional board reads and separately approved `project` scope plus config opt-in for writes | User-supplied token resolved via Keychain for probe; viewer/org IDs observed; helper bounds and principal-gate are TASK-142.2 | Local CLI and explicit checkout; effects require consent |
...
| Seam | Decision |
| --- | --- |
| Source adapters | Built-in Go interfaces and stable external JSON-RPC v1 stdio (D16). `queue adapter protocol --json` checks host majors, `queue adapter check --executable PATH --json` probes an unapproved binary, and `queue init --adapter external --executable PATH --json` negotiates its manifest and proposes schema-checked owner-private YAML; `queue adapter approve --source ID --acknowledge` is separate. An explicitly mapped external `start` can run through `queue next --claim --start` with a worker claim, guarded write and receipt read-back; GitHub writes stay interactive-only. See the [authoring guide](../skills/worklease-workflow/references/external-adapter-authoring.md). No SDK or generic MCP bridge ships. |
...
| Credential helpers | Built-in `gh` remains for GitHub. A bounded argv helper ships with Linear and is reused by Jira Cloud and external adapters; for external sources, an approved account and canonical HTTPS `config.origin` bind the host-resolved token, delivered before each operation by the optional `host-credential-v1` protocol method. No raw token in queue.yaml. |
...
| Executable policy plugins, widgets, lifecycle hooks, storage plugins, event bus, SDKs | Out of scope. Protocol v1 needs no language-specific SDK, and hooks cannot replace recoverable receipts or deterministic resource keys. |

**External adapter protocol.** The [v1 wire contract](external-adapter-protocol.md) and [versioned message schemas](external-adapter/v1/schema.json) refine D16 and this section. JSON-RPC 2.0 over stdio uses newline-delimited messages. This is the same framing as MCP stdio, so existing libraries work, but it uses a Worklease-specific method set that mirrors §7. MCP tools are not used as the adapter protocol because they carry no standard pagination, capability, coverage, or receipt semantics; mapping them is exactly the per-provider adapter work. An external adapter may itself call a provider's MCP server or API. The boundary needs:
...
- Source-scoped credential references or the shipped `host-credential-v1` helper channel. A configured `credentialHelper`, account, and canonical HTTPS `config.origin` bind credential delivery before each operation; the adapter verifies identity and scope. Never send Worklease bearer credentials. Minimize inherited environment and configuration.
- A fake provider, golden fixtures, identity vectors, and tests for pagination, stale writes, capability denials, cancellation, quota handling, malformed output, uncertain outcomes, and secret leakage. The shipped `queue adapter check` exposes bounded pass/fail/skip and stable exit codes for any explicitly selected executable; write and cancellation probes need disposable targets/markers. In-repo tests also exercise built-ins.

The first host implementation restricts external sources to the built-in `generic` resource policy. Read-only sources need no claim binding; claiming and writing require an explicit, agreed `claims: {policy: generic, source: ...}` in `queue.yaml`, confirmed through the existing identity gate. `worklease queue adapter approve --source ID --acknowledge` records the executable digest, expected identity/version, principal, workflow, claim binding, and source-config/credential-reference hash in owner-private state. A changed binding or executable requires reapproval; the host executes a private copy of the approved bytes, not the configured pathname. Each configured source has one lazily started supervised process; protocol failures affect that source alone, and journaled writes use `readReceipt` for recovery rather than redispatch. Other static policies are not implicit host support. The host-resolved credential channel is explicit and requires a configured, approved helper and adapter-declared support.
...
The configured launcher must have a tested handoff contract that consumes the exact resources and verifies the authority ID before work. `scripts/queue-launch-worker.py` is a reference: from the configured cwd it compares `worklease queue authority-id --json` against `WORKLEASE_QUEUE_AUTHORITY_ID`, acquires every passed resource under a new worker session, and verifies exact membership through that session's private handle. If the worker uses a non-default `WORKLEASE_HOME`, name it in `passEnv`. Passing `WORKLEASE_QUEUE_RESOURCES` does not change the existing CLI's key policy automatically; arbitrary commands may ignore it. Do not report a coordinated worker merely because process launch succeeded. API-only sources need an explicitly configured working directory when `{checkout}` is unavailable; unresolved placeholders disable the action. Argv substitution prevents shell expansion, not every target program's option parsing: use explicit value arguments or `--` where appropriate and test option-like identifiers.
...
### Linear loading (TASK-142.5)

- The per-organization/account serial quota queue schedules issue pages as background work and checks both request and complexity reset headers before the next call. Relay pages are capped at 250, and a refresh reads at most five issue pages before retaining its cursor. A principal/team/project/token-bound, metadata-only SQLite partition records the cursor and a fixed scan-start window boundary; no Linear payload is seeded offline. A fresh process re-verifies the viewer and begins with a full traversal rather than trusting an old partial projection.
- Once a traversal ends without interruption, subsequent windows use the probed `updatedAt.gte` filter with a 30-day overlap. The committed **window** timestamp means only that this traversal exhausted its cursor; it never certifies complete source membership or absence. Moving `updatedAt` pages and unprobed permission visibility keep source coverage partial even after the last page. A failed/cancelled page or cursor loop does not advance the window timestamp; missing rows are never deleted or classified inaccessible from scan absence. Archive/trash remain separate visible states, not deletion. Gaps older than the overlap remain possible and cannot establish complete-source membership.
- Relation directions are fetched independently of endpoint `updatedAt`, including a persisted, round-robin sweep of up to 50 known issues per finished window. Both directions and all pages must succeed before replacing an item's prior complete edge set; a partial read makes its closure unknown. With 10,000 unchanged issues, a full sweep needs at least 200 finished windows and roughly 20,000 relation requests, excluding issue pages/details. This is a quota/latency limit, not a claim of instantaneous complete graph coverage. A selected issue still refreshes its exact closure on demand.
- **Linear claim selection (TASK-142.6):** a finished traversal whose only coverage gap is unprovable source membership may select *observed* Linear UUID issues, not assert that the source is complete or that no other work exists. Every observed item must have a fresh complete dependency closure; before acquisition, re-read the selected issue and its full prerequisite closure, check the confirmed organization/issue key and admission, and observe the exact authority resource again. Interrupted scans, stale or unknown closures, and other incomplete source types still stop selection. `queue next`/MCP returns `selectionScope: known-linear-items` alongside partial source coverage; when no observed candidate is acquired it reports `incomplete`, never complete-and-empty. Claim for me uses the same per-item checks. Identity confirmation verifies the organization and viewer rather than requiring a complete Linear list, which its visibility model cannot provide. Unknown issues can be missed by discovery; a claim is coordination only, not a guarantee of globally earliest work or provider exclusion.
- D21 deterministic localhost fixture, Apple M1 Max, `go test -run '^$' -bench BenchmarkLinearSync10000 -benchtime=1x -count=3 ./internal/queue`: three 10,000-item traversal + 50-item relation-sweep samples took 197/263/470 ms (one sample each; not p95). Each sample made 40 list + 100 relation HTTP requests, with ~55–56 MiB Go allocations; no network latency, API quota charging, provider response variation, SQLite publication, detail reads, or multi-client load was injected. Unlike the earlier GitHub 100-page first-view assumption, Linear uses 40 pages per 10k traversal but relation cost dominates: 20k relation calls for a full 10k sweep plus up to 10k detail reads. At the probed 2,500-request per-window quota, a full relation sweep spans at least eight quota windows before other traffic. Do not extrapolate the localhost timing to production or treat the 30-day overlap as lossless history.
...
Measure on the D22 reference machine with fixed fixtures and injected network latency. Report p50, p95, and p99, process count, API request count, bytes transferred, RSS, disk and index size, and quota cost. The observable render boundary is bytes written by Bubble Tea to a PTY, not pixels painted by a terminal emulator; physical display paint requires terminal-specific instrumentation and is not claimed by these budgets. Fixture-only operations are labeled separately from provider-to-index work. The per-row accounting, measurement scope, and reproducible commands are in [queue benchmark results](queue-benchmark-results.md). Provider-controlled latency is never a Worklease guarantee.

| Workload | Target |
| --- | --- |
| Warm first view, 10,000 summaries | **Met at the Go View boundary:** 20-sample p50/p95/p99 55.93/58.55/60.56 ms with no network; standalone 10k first View p95 55.70 ms. Physical terminal display paint remains outside the stated render boundary. |
| Navigation and scrolling | **Met for cached rows, revised under full refresh:** 10k PTY keyboard-to-renderer-output p50/p95/p99 16.65/17.95/18.38 ms; 50k combined-fault publication+input-to-output 49.26/56.09/56.63 ms. Revise the concurrent 50 ms p95 target to **75 ms p95** for the measured 50k full-refresh fixture: synchronous snapshot reconciliation and concurrent SQLite index work cause a measured 56.09 ms tail. Do not weaken claim renewal to achieve it; the minimum previous-lease margin was 9m36.39s at a 10m TTL. The current 100k full-refresh p95 remains above even this revised target (87.50 ms PTY, without provider/index load): that is the stress failure boundary, not a passing concurrent-refresh guarantee. |
| Indexed filter or search, 10,000 summaries | **Met for indexed FTS:** 20-sample p50/p95/p99 57.21/67.80/67.80 ms, 26,106,408-byte index, zero external processes per keystroke. The initial index build is excluded. |
| Cold remote view | **Met from page return to running PTY renderer output:** 20-sample p50/p95/p99 9.99/11.25/11.44 ms with 5 ms injected HTTP latency; request-to-output was 16.33/18.18/18.28 ms. This is the first 100 summaries of a 10k fake GitHub source; it does not include initial authentication, new-process startup, SQLite publication or physical display paint. Full enumeration took 598.95/621.69/629.01 ms, so the first page did not wait for it. |
| View of 50,000 summaries across sources | Navigable, within a provisional 512 MiB RSS budget excluding provider processes, which are reported separately. The original 256 MiB provisional target fails: the prior unprepared refresh measured p95 peak RSS 450,887,680 bytes (430 MiB); the current worker-prepared path measured p95 362,987,520 bytes (346 MiB) across 20 standalone D22 processes. The 512 MiB revision leaves room above the latest 20-process p95 373,719,040 bytes (356 MiB); **met for the standalone queue model**. Provider/index concurrency is measured separately in a fixture test process (1,118,486,528 bytes including the graph and server), so a production multi-process total is not asserted. Physical terminal paint is outside the render boundary. |
| 100,000-summary browsing stress | **Met for warm bounded-viewport navigation; refresh limit explicitly revised:** warm PTY navigation p95 18.23 ms and Go Update+View p95 2.35 ms. The 100k failure boundary is a simultaneous full refresh: the latest standalone prepared refresh+View p50/p95/p99 is 80.54/86.14/86.83 ms with p95 RSS 653,344,768 bytes (623 MiB); a separate PTY refresh+key run reached p95 101.92 ms, beyond the revised 75 ms 50k concurrency target. Revise the 100k stress expectation to browsing with refresh responsiveness reported as degraded, not a promise of the 50k input target. The earlier unprepared 100k path reached p95 878 ms and 855,064,576 bytes; neither path crashed. The displayed viewport is bounded, while fixture preparation and snapshot storage still scale with the full corpus. |
...
| Rate limiting, a hung adapter, a large graph, and a full refresh together | **Met at the PTY renderer/local-authority boundary:** 20-sample 50k combined run drove a real adapter through fake HTTP 429, held another adapter HTTP request until cancellation, recomputed a 50k-node graph, replaced a 10k-row SQLite index, published prepared full snapshots and renewed a local claim. Publication+input-to-output p50/p95/p99 49.26/56.09/56.63 ms; renewal 1.16/3.22/73.47 ms, minimum previous-lease margin 9m36.39s at 10m TTL. All samples produced fresh output; neither input nor renewal froze. The production provider-to-index scheduler and remote renewal are not exercised here; no guarantee extends to those unmeasured paths. |
...
#### Remote authority load (TASK-129.7, 2026-09-23; rerun 2026-09-24)

On D22 (MacBookPro18,2; M1 Max, 32 GiB), a disposable real TLS `worklease serve` ran with 25 separately enrolled clients and 500 active 10-minute claims. Each client kept one namespace watch and refreshed a batch-status projection of its 20 claims after every watch response, as the D20 overlay does; the harness used the default 250 ms server polling interval. Evidence: `scripts/authority-queue-benchmark.py` and the retained private `/private/tmp/worklease-authority-review-full-2/report.json`. The run renewed all 500 claims around 65 seconds after acquisition, before half the TTL, then required every client to consume all 500 `renewed` events and reach the head (an empty watch) before accepting the result. It measured the authority separately from provider traffic. Reproduce with `mise run authority-benchmark` (builds the binary; retains an owner-private `/tmp/worklease-authority-*` directory with credentials; inspect and dispose of that directory securely). For a fast correctness smoke use `python3 scripts/authority-queue-benchmark.py --binary ./bin/worklease --output /tmp/worklease-authority-smoke-UNIQUE --clients 2 --claims 4 --warmup-seconds 2`; `--output` must be a nonexistent directory.

| Metric | Observed |
| --- | --- |
| Burst acquire, 500 claims (p50/p95/p99) | 467/846/1,143 ms per operation, 25 concurrent clients |
| Renewal write latency, 500 claims (p50/p95/p99) | 677/909/990 ms per operation; p99 **previous lease** remaining TTL at completion 520 s, minimum 520 s (target ≥300 s); 25 watches active at renewal start and end |
| Overlay catch-up after the 500-renewal burst | Every client observed all 500 `renewed` events with no history gap; p50/max 200/201 s to drain to head. The watch returns one event per response, so catch-up is linear in burst size. With these CLI-subprocess clients each event costs a watch plus a status process start (~0.4 s); see the in-process estimate below |
| Watch and overlay traffic | 13,959 namespace-watch responses plus 13,959 batch-status reads over 280 s; 23,215 **modeled** store polls (~83/s) versus 25 × 4 = 100/s steady polling. The authority has no store-read counter, so the polling rate is derived from wall time and the configured interval, not measured |
| Authority CPU and memory | CPU time 1.42 → 66.95 s over the 280 s watch/renew/catch-up window (~65.5 CPU seconds, ~23% of one core, ~2.3 ms per request including watch polling); RSS 36,368 → 43,216 KiB |
| Authority database and WAL | database 1,081,344 → 1,449,984 bytes; WAL 4,190,072 → 4,540,272 bytes (+350,200 bytes at sampled endpoints; not a long-term growth guarantee) |
| Shorter TTL saturation | First 500 changes to 30 s succeeded (p50/p95/p99 495/723/871 ms); another 500 renewals five seconds later succeeded (492/705/809 ms), with at least 13.9 s remaining on the preceding 30 s leases. At this two-round burst, shorter TTL did not saturate the authority; this is not a sustained 30 s capacity guarantee. |

At this measured 25-client/500-claim load, watches did not saturate the store: renewal p99 with all 25 watches active stayed far inside the 10-minute safety margin, every overlay drained without a gap, and the authority used about a quarter of one core. Catch-up time is the scaling cost of one event per watch response. The 200 s figure is dominated by per-call process startup and TLS setup in the CLI harness; at the measured ~2.3 ms of authority CPU per request, an in-process queue client needs two requests per event, so a 500-event burst is estimated at seconds, not minutes. That estimate is not measured. Queue-owned renewals are jittered per claim (§9), so steady-state traffic at this scale is about two events per second rather than a 500-event burst. Overlay staleness never authorizes an action: every claim, launch, and write re-verifies against the authority first. Do not infer a capacity guarantee from claim count alone: renewal cadence, bursts, filesystem and network conditions matter. The harness raises metadata and enrollment quotas only on its disposable server so subprocess startup cannot mask heartbeat results; it does not change production defaults. No server-side watch-coalescing work is justified by this run. If a future overlay-freshness budget requires faster burst catch-up, multi-event watch responses are the lever. The two 30-second renewal rounds succeeded, but longer saturation runs are needed before promising continuous 30-second renewals.
...
- **S8 delivered — Beads:** TASK-141 probes and pins `bd` 1.3.0, adds an embedded-Dolt built-in adapter, portable generic binding, bulk typed edges, periodic reconciliation, and recoverable focused writes. No new resource policy or native Beads claim is introduced (D31).
- **S8 accepted — GitHub Projects v2 status:** user request on 2026-09-25 superseded the S6 deferral. TASK-140's user-approved synthetic-project probe (§3) established single-select discovery, distinct project-item identity, independent field update signals, required `read:project`/`project` scopes, and a cost of one per sampled query. The optional binding and separately authorized unconditional field writes extend the existing GitHub adapter without changing claim keys or issue completion (D32). GHES remains unprobed.
...
- **S6 scope at delivery:** GitHub Projects v2 status mapping was deferred because it requires project scope and per-project field discovery. The subsequent S8 request accepted it as TASK-140 (D32); the synthetic probe above resolves the read/write scope and update-signal questions. No label-based workflow state is inferred.
