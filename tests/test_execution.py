...
class ExecutionTests(unittest.TestCase):
    ...
    def test_exec_uses_argv_captures_output_and_replays_without_side_effect(self) -> None:
        ...
        first, first_code = execute(
            self.store, request, [sys.executable, "-c", script]
        )
        retry, retry_code = execute(
            self.store, request, [sys.executable, "-c", script]
        )
        ...
        with self.assertRaisesRegex(LeaseError, "operation-id-request-mismatch"):
            execute(
                self.store, request, [sys.executable, "-c", "print('changed')"]
            )
    ...
    def test_exec_heartbeats_during_long_child_and_returns_failure_status(self) -> None:
        ...
        receipt, code = execute(
            self.store,
            request,
            [sys.executable, "-c", "import time; time.sleep(0.45); print('done'); raise SystemExit(7)"],
        )
